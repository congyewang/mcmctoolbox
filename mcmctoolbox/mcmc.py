...
class MCMC:
    ...
    def fisher_adaptive_mala(self, lambda_=10, alpha_star=0.574, n0=500, rho_n=0.015):
        ...
        def compute_R_n(s_n, R_prev):
            ...
            R_n = R_prev - r_n * np.outer(np.dot(R_prev, phi_n), phi_n) / (1 + np.dot(phi_n, phi_n))
            return R_n
        ...
        for _ in range(n0):
            ...
            x_prime = theta_curr + (sigma2 / 2) * self.grad_log_pi(theta_curr) + np.sqrt(sigma2) * eta
        ...
        R = np.eye(self.d)
        sigma_R2 = sigma2
        log_pi_curr, grad_log_pi_curr = self.log_pi(theta_curr), self.grad_log_pi(theta_curr)
        ...
        for i in range(self.nits):
            ...
            theta_prop = theta_curr + (sigma_R2 / 2) * np.dot(R, np.dot(R.T, grad_log_pi_curr)) + np.sqrt(sigma_R2) * np.dot(R, eta)
            log_pi_prop, grad_log_pi_prop = self.log_pi(theta_prop), self.grad_log_pi(theta_prop)
            ...
            alpha = min(1, np.exp(log_pi_prop + h_curr_prop - log_pi_curr - h_prop_curr))
    ...
    def am_mh(self, initial_mu=None, lambda_initial=1, gamma_sequence=None, alpha_star=0.234):
        """
        ...
        for i in range(self.nits):
            ...
            theta_prop = np.random.multivariate_normal(theta[i], lambda_seq[i] * Sigma[i])
            ...
            # Update parameters
            lambda_seq[i+1] = np.exp(np.log(lambda_seq[i]) + gamma_sequence[i] * (np.exp(log_alpha) - alpha_star))
            mu[i+1] = mu[i] + gamma_sequence[i] * (theta[i+1] - mu[i])
            Sigma[i+1] = Sigma[i] + gamma_sequence[i] * (np.outer(theta[i+1] - mu[i], theta[i+1] - mu[i]) - Sigma[i])
